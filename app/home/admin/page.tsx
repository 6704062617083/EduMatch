    ...
    <div className="min-h-screen bg-orange-50 font-sans tracking-tight antialiased flex flex-col">
      <div className="flex justify-between items-center px-10 py-5 bg-[#FC5404] text-white shadow-md">
        <div className="flex items-center gap-4">
          <Image src="/Edu_icon.png" alt="Logo" width={120} height={35} className="object-contain" />
          <div className="h-6 w-[1px] bg-white/30 ml-2"></div>
          <span className="text-lg font-black tracking-tighter uppercase">Admin Dashboard</span>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8 max-w-[1600px] mx-auto w-full">
          <div className="flex flex-col">
            ...
            <div className="space-y-4">
              {tutorData.length === 0 && (
                <div className="bg-white rounded-[32px] p-16 text-center border-2 border-orange-100 border-dashed text-gray-300 font-bold">ไม่มีรายการรอตรวจสอบติวเตอร์</div>
              )}
              ...
              {tutorData.map((item) => {
                ...
                return (
                  <div key={item._id} className={`bg-white border rounded-[28px] overflow-hidden transition-all duration-300 ${isOpen ? "shadow-xl ring-1 ring-orange-200" : "shadow-sm hover:shadow-md border-orange-100"}`}>
                    <div onClick={() => setExpandedTutor(isOpen ? null : item._id)} className="p-5 flex justify-between items-center cursor-pointer hover:bg-orange-50/30 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-lg uppercase">
                          {item.userId?.name?.[0]}
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-gray-400 font-medium tracking-wide">{item.userId?.email}</p>
                        </div>
                      </div>
                      <span className={`px-4 py-1.5 rounded-xl text-[11px] font-extrabold border uppercase tracking-wider ${st.color}`}>{st.label}</span>
                    </div>

                    {isOpen && (
                      <div className="p-6 pt-0 border-t border-orange-50 bg-white">
                        ...
                        <div className="mt-8">
                          <h4 className="text-[13px] font-black text-[#1e3a5f] uppercase tracking-widest mb-4">เอกสารประกอบการพิจารณา</h4>
                        ...
                        {item.status === "rejected" && item.rejectReason && (
                          <div className="mt-6 bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700">
                            <p className="text-xs font-bold uppercase tracking-wider mb-1">เหตุผลที่ไม่ผ่าน</p>
                        ...
                        {item.status === "pending" && (
                          <div className="flex gap-3 mt-8">
                            <button onClick={() => updateTutorStatus(item._id, "approved")} className="flex-1 bg-green-500 hover:bg-green-600 text-white font-bold py-3.5 rounded-2xl transition-all shadow-lg shadow-green-100 active:scale-95">
                              อนุมัติ
                            </button>
                            <button onClick={() => updateTutorStatus(item._id, "rejected")} className="flex-1 bg-white border-2 border-red-100 text-red-500 hover:bg-red-50 font-bold py-3.5 rounded-2xl transition-all active:scale-95">
                              ปฏิเสธ
                            </button>
                          </div>
                        )}
          ...
          <div className="flex flex-col">
            ...
            <div className="space-y-4">
              {[...pendingPayments, ...paidPayments].length === 0 && (
                <div className="bg-white rounded-[32px] p-16 text-center border-2 border-orange-100 border-dashed text-gray-300 font-bold">ไม่มีรายการสลิป</div>
              )}
              ...
              {[...pendingPayments, ...paidPayments].map((item) => {
                ...
                return (
                  <div key={item._id} className={`bg-white border rounded-[28px] overflow-hidden transition-all duration-300 ${isOpen ? "shadow-xl ring-1 ring-blue-100" : "shadow-sm border-orange-100"}`}>
                    <div onClick={() => setExpandedPay(isOpen ? null : item._id)} className="p-5 cursor-pointer hover:bg-orange-50/30 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <p className="font-black text-orange-500 text-lg">฿{item.price.toLocaleString()}</p>
                      </div>
                      <h3 className="font-bold text-[#1e3a5f]">
                        {item.studentId?.name} {item.studentId?.surname}
                      </h3>
                      <p className="text-xs text-gray-400 font-medium truncate">{item.courseId?.title}</p>
                    </div>
                        ...
                        <div className="mt-4 space-y-4">
                          <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
                            ...
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="text-[#1e3a5f] font-bold text-right ml-4 leading-tight">{item.courseId?.title}</span>
                            </div>
                          </div>
                          ...
                          {item.paymentStatus === "slip_uploaded" && (
                            <div className="flex gap-2 pt-2">
                              ...
                              <button
                                ...
                                className="bg-white border-2 border-red-100 text-red-500 font-bold px-6 py-3.5 rounded-2xl active:scale-95 transition-all"
                              >
                                ไม่ผ่าน
                              </button>
                            </div>
                          ...
                          {item.paymentStatus === "paid" && (
                            <div className="space-y-4 pt-4 border-t border-orange-50">
                              {item.tutorQr && (
                                <div className="space-y-2">
                                  <p className="text-[11px] font-black text-[#1e3a5f] uppercase tracking-widest">QR โอนเงินให้ติวเตอร์:</p>
...
function Field({ label, value }: { label: string; value?: any }) {
  return (
    <div className="flex flex-col">
      ...
      <span className="text-[14px] text-[#1e3a5f] font-bold leading-tight">{value || "-"}</span>
    </div>
  );
}
...
function DocLink({ url, label }: { url?: string; label: string }) {
  ...
  return (
    <a
      href={url}
      target="_blank"
      className="px-3 py-2 bg-white border border-orange-100 hover:border-orange-400 hover:text-orange-600 rounded-xl text-xs font-bold text-gray-500 transition-all flex items-center gap-2 shadow-sm"
    >
